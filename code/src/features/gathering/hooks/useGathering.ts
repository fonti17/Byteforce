    ...
    async (
      ...
      try {
        const turn = await gatheringService.process(
          text,
          { data, originalRequest, expectedField: field },
          optionsRef.current
        );
        ...
        // Apertus is unreachable — the deterministic extractor keeps the app usable,
        // and `source: 'local'` is what tells the UI the read was made without it.
