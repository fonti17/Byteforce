...
/** Course values allowed by `recipe.config.json`. */
...
/** Diet flags allowed by `recipe.config.json`. */
...
/**
 * Mirrors `recipe.config.json`, including `additionalProperties: false`.
...
/**
 * Required properties of `recipe.config.json` that can still be open on a
