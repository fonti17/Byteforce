...
import type { Recipe, RecipeLibraryFile, RecipeOptions, StoredRecipe } from '../types';
...
export function useRecipes(options: RecipeOptions = {}) {
  ...
  useEffect(() => {
    let isActive = true;
    void recipeStore.list().then((stored) => {
   ...
   * `recipe.config.json` describes it — which is what the detail screen
   * downloads.
   */
  ...
  const selectedRecipes = useMemo(
    () => recipes.filter((entry) => selectedIds.includes(entry.id)).map((entry) => entry.recipe),
    [recipes, selectedIds]
  );
